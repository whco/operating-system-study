...
- 두 프로세스가 ~~병렬로~~ "병행"으로 실행, 병렬로도 가능.
  - 병렬로 실행하는 데 왜 context switching 발생하지????
    ...
    >     ``` Interrupt : 주로 하드웨어를 통한 I/O 요청이나, OS / Driver 레벨의 Timer 기반 스케쥴링에 의해 발생 ```<br>
...
> > <image src = "https://user-images.githubusercontent.com/57519837/143763676-151a0cac-fba1-49ef-a378-e7a7d629f915.png" width="30%" height="30%"> <br>
> > - blocking / non-blocking은 호출되는 함수가 바로 리턴하느냐 마느냐 : x / o
> > - sync / async  는 호출되는 함수의 작업 완료 여부를 누가 신경쓰냐 : 호출하는 함수 / 호출되는 함수
> 싱글 스레드 서버 : 
